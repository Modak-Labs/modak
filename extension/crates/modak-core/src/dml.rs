...
fn halves(meta: &TableMeta, t: TierKey, frag: &DmlFragments) -> Result<Halves> {
    ...
    let gate = "(SELECT count(*) FROM __modak_cold) IS NOT NULL";
...
/// Wraps the tier key in `modak_cold_dml`, which checks the retention floor,
...
fn guarded_tier(tier_expr: &str, retention: Option<i64>, frag: &DmlFragments) -> String {
    ...
    format!("modak_cold_dml({tier_expr}, {r}, {stash})")
}
...
pub fn render_update(
    ...
    if frag.set_items.is_empty() {
        return Err(ModakError::Planning("UPDATE with no SET items".into()));
    }
    ...
    let mut projection = meta
        ...
        .collect::<Vec<_>>();
    if moves_tier {
        projection.push(format!("m.{tier} AS __modak_old_tier"));
    }
...
pub fn render_delete(
    ...
    let h = halves(meta, t, frag)?;
    Ok(format!(
        "WITH __modak_cold AS (\n\
           INSERT INTO modak.delta AS d \
             (table_id, pk, op, tier_key, old_tier_key, version, payload)\n\
           SELECT {table_id}, {pk}, 1, {guarded}, NULL::bigint,\n\
                  nextval('modak.delta_version'), jsonb_build_object({pk_pairs})\n\
...
mod tests {
    ...
    fn update_splits_into_delta_cte_and_bounded_heap_update() {
        ...
        assert!(sql.starts_with("WITH __modak_new AS ("), "{sql}");
        assert!(!sql.contains("__modak_move"), "no tier move here:\n{sql}");
        assert!(
        ...
        assert!(
            sql.contains("modak_cold_dml(m.\"event_time\", NULL::bigint, NULL::text[])"),
            "{sql}"
        );
        assert!(
        ...
        assert!(
            sql.ends_with(
                "UPDATE \"public\".\"events\" m SET \"val\" = ('y') \
                 ...
                 AND (SELECT count(*) FROM __modak_cold) IS NOT NULL"
            ),
            "{sql}"
        );
    }
    ...
    fn update_returning_stashes_new_image_expressions() {
        ...
        assert!(
            sql.contains(
                "modak_cold_dml(m.\"event_time\", NULL::bigint, \
    ...
    fn delete_writes_tombstones_with_pk_payload() {
        ...
        assert!(
            sql.contains(", 1, modak_cold_dml(m.\"event_time\", 40, NULL::text[]),"),
            "{sql}"
        );
    ...
    fn delete_returning_stashes_old_image_expressions() {
        ...
        assert!(
            sql.contains("modak_cold_dml(m.\"event_time\", NULL::bigint, ARRAY[(val)::text])"),
            "{sql}"
        );
    ...
    fn set_tier_key_decomposes_into_move_ctes() {
        ...
        let sql = render_update(&meta(), T, None, &frag).unwrap();
        assert!(
            sql.contains("m.\"event_time\" AS __modak_old_tier"),
        ...
        assert!(
            sql.contains(
                "CASE WHEN m.\"event_time\" >= 100 THEN m.__modak_old_tier \
        ...
        assert!(
            sql.contains(
                "CASE WHEN m.\"event_time\" < 100 \
                 THEN NULLIF(m.__modak_old_tier, m.\"event_time\") END"
            ),
            "cold moves remember where the lake holds the image:\n{sql}"
        );
        assert!(
            sql.contains("to_jsonb(m) - '__modak_old_tier'"),
        ...
        assert!(
            sql.contains(
                "__modak_move AS (\n\
                 ...
                 SELECT \"id\", \"event_time\", \"val\" FROM __modak_new m \
                 WHERE m.\"event_time\" >= 100"
            ),
