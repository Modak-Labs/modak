...
SELECT modak_delete('public.events'::regclass, '3', 110);
```

The router compares the record's tier key against the cut-line. Recent rows go to the heap as ordinary DML, cold rows become `modak.delta` entries (upsert or tombstone). Every read merges the delta over the pinned snapshot, newest wins, so a correction is visible immediately, long before it reaches Iceberg.
...
- `MODAK_COMPACTION_BATCH` (default 1000) bounds rows folded per cycle.

A persistently growing backlog (`modak_delta_backlog_rows`) usually means a long-held read pin is blocking the fold. See [Operations](../operations/day-2.md#backpressure).
...
Anything else, such as `DROP COLUMN`, `RENAME COLUMN`, `ALTER TYPE` across type families, or `TRUNCATE` on a mirrored table, cannot be applied to the lake without rewriting history. Modak fails that table loudly instead of diverging silently:
